...
help () {
    ...
    echo "This program compares the running time of 6 sorting algorithms:"
    echo "  -Bubble Sort"
    echo "  -Insertion Sort"
    echo "  -Selection Sort"
    echo "  -Shell Sort"
    ...
    echo ""
    echo "Examples: Note that the arguments of the -n flag must be enclosed in quotes if there are more than one."
...
while getopts ":s:t:n:g:a:h" opt; do
  case ${opt} in
    s )
        type=$OPTARG
        ;;
...
kotlin -cp $LIBDIRS MainKt $type $attempts ${#seqSizes[@]} ${seqSizes[@]} $algName $plotFileName